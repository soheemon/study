...
public class BankSystem {
	...
	public BankSystem() {
		myBank = new MyBank(); //MyBank 객체 생성
		showMenu(); // showMenu 실행
	}
	...
	public void showMenu() { // show method 만들기
		...
		do {
			...
			if (menu.equals("1")) { // 고객등록
				...
				myBank.addCustomer(id, name, balance); //add Customer하면서 Account를 생성해야할듯

			} else if (menu.equals("2")) { // 고객찾기
			...
			} else if (menu.equals("3")) { // 고객 전체보기

				Customer[] allCust = myBank.getAllCustomers(); // 배열 개수찾기.
				for (int i = 0; i < MyBank.getCustomersNum(); i++) {
					System.out.println(allCust[i].getId() + ": " + allCust[i].getName() + " :"
							+ allCust[i].getAccount().getBalance());
				}

			} else if (menu.equals("4")) { // 예금 출금
